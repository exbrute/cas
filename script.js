    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if ("win" in obj) {
                ...
                $(that).addClass("mines-sq-success2").removeClass("mines-sq");
                if (obj.win.finish == 1) {
                    $("#buttonFinishMines").removeClass("disabled").hide();
                    $("#buttonFinishMinesMobile").removeClass("disabled").hide();
                    $("#buttonStartMines").show();
            ...
            if ("lose" in obj) {
                ...
                $(".bd-primary").removeClass("").addClass("lose-bl");
...
function bet(type) {
    if ($("#userBalance").html() < $("#diceGameAmount").val()) {
        $("#error_bet").html("РќРµРґРѕСЃС‚Р°С‚РѕС‡РЅРѕ СЃСЂРµРґСЃС‚РІ");
        return $("#error_bet").show();
    }
    if ($("#diceGamePercent").val() > 95 || $("#diceGamePercent").val() < 1) {
        $("#error_bet").html("% РЁР°РЅСЃ РѕС‚ 1 РґРѕ 95");
        return $("#error_bet").show();
    }
    $.ajax({
        type: "POST",
